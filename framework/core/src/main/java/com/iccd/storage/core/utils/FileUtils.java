package com.iccd.storage.core.utils;
...
public class FileUtils {
    ...
    public static String getFileSuffix(String filename) {
        if (StringUtils.isBlank(filename) || filename.lastIndexOf(StorageConstants.POINT_STR) == StorageConstants.MINUS_ONE_INT) {
            return StorageConstants.EMPTY_STR;
        }
        return filename.substring(filename.lastIndexOf(StorageConstants.POINT_STR)).toLowerCase();
    }
    ...
    public static String getFileExtName(String filename) {
        ...
        return filename.substring(filename.lastIndexOf(StorageConstants.POINT_STR) + StorageConstants.ONE_INT).toLowerCase();
    }
    ...
    public static String generateStoreFileChunkRealPath(String basePath, String identifier, Integer chunkNumber) {
        return new StringBuffer(basePath)
                ...
                .append(File.separator)
                .append(UUIDUtil.getUUID())
                .append(StorageConstants.COMMON_SEPARATOR)
                .append(chunkNumber)
                .toString();
    }
    ...
    public static void writeFile2OutputStream(FileInputStream fileInputStream, OutputStream outputStream, long length) throws IOException {
        ...
        fileChannel.transferTo(StorageConstants.ZERO_LONG, length, writableByteChannel);
        outputStream.flush();
    ...
    public static void writeStream2StreamNormal(InputStream inputStream, OutputStream outputStream) throws IOException {
        ...
        int len;
        while ((len = inputStream.read(buffer)) != StorageConstants.MINUS_ONE_INT) {
            outputStream.write(buffer, StorageConstants.ZERO_INT, len);
        }
        outputStream.flush();
        inputStream.close();
        outputStream.close();
