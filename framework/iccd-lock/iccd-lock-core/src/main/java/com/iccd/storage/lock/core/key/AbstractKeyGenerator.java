...
import com.iccd.storage.core.utils.SpElUtil;
import com.iccd.storage.lock.core.LockContext;
import com.iccd.storage.lock.core.annotation.Lock;
...
public abstract class AbstractKeyGenerator implements KeyGenerator {
    ...
    public String generateKey(LockContext lockContext) {
        ...
        if (ArrayUtils.isNotEmpty(keys)) {
            Arrays.stream(keys).forEach(key -> keyValueMap.put(key, SpElUtil.getStringValue(key, lockContext.getClassName(), lockContext.getMethodName(), lockContext.getClassType(),
