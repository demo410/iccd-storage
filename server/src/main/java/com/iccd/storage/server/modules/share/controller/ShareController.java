package com.iccd.storage.server.modules.share.controller;
...
import com.iccd.storage.server.common.annotation.NeedShareCode;
...
import com.iccd.storage.server.modules.share.context.*;
import com.iccd.storage.server.modules.share.convert.ShareConverter;
import com.iccd.storage.server.modules.share.po.CancelSharePO;
import com.iccd.storage.server.modules.share.po.CheckShareCodePO;
import com.iccd.storage.server.modules.share.po.CreateShareUrlPO;
import com.iccd.storage.server.modules.share.po.ShareSavePO;
import com.iccd.storage.server.modules.share.service.IShareService;
...
import com.iccd.storage.server.modules.share.vo.ShareUrlListVO;
import com.iccd.storage.server.modules.share.vo.ShareUrlVO;
...
public class ShareController {
    ...
    public R<ShareUrlVO> create(@Validated @RequestBody CreateShareUrlPO createShareUrlPO) {
        ...
        List<Long> shareFileIdList = Splitter.on(StorageConstants.COMMON_SEPARATOR).
    ...
    public R<List<ShareUrlListVO>> getShares() {
        ...
        List<ShareUrlListVO> result = iShareService.getShares(context);
    ...
    public R cancelShare(@Validated @RequestBody CancelSharePO cancelSharePO) {
        ...
        List<Long> shareIdList = Splitter.on(StorageConstants.COMMON_SEPARATOR).splitToList(shareIds).stream().map(IdUtil::decrypt).collect(Collectors.toList());
    ...
    public R<List<UserFileVO>> fileList(@NotBlank(message = "文件的父ID不能为空") @RequestParam(value = "parentId", required = false) String parentId) {
        ...
        List<UserFileVO> result = iShareService.fileList(context);
        return R.data(result);
    }
